...
// 부제/핵심 포인트)로 압축(검토/수정 가능한 textarea로 노출) -> 2) 그
...
async function generateCardNewsSummary() {
  ...
  try {
    ...
    const prompt = `
아래 뉴스 기사를 "한 장짜리 카드뉴스(인포그래픽)"에 들어갈 텍스트로 정리하십시오. 여러 장의 슬라이드가 아니라, 이미지 한 장 안에 제목·핵심 수치·핵심 포인트가 한눈에 들어오도록 구성하는 것이 목적입니다.
...
[출력 형식 -- 반드시 이 형식 그대로, 대괄호 라벨을 포함해서 출력]
[헤드라인]
(한 줄. 15~25자 내외. 이 기사에서 가장 임팩트 있는 사실 하나로 압축. 날짜·수치가 핵심이면 그대로 포함)

[부제]
(헤드라인을 보완하는 한 문장. 20~40자 내외. 마땅한 내용이 없으면 이 항목 자체를 생략)

[핵심 포인트]
- (소제목: 짧은 설명 형태. 소제목은 2~6자, 설명은 15~30자 내외. 예: "SRT: 수도권↔전국 주요 도시 빠르게 연결")
- (소제목: 짧은 설명)
- (소제목: 짧은 설명)
(기사에서 실제로 중요한 사실만 3~5개. 각 항목은 그 자체로 완결된 정보 단위여야 함)

[마무리 한 줄]
(전체 내용이 독자에게 어떤 의미인지 짚어주는 한 문장. 20~40자 내외. 마땅한 내용이 없으면 이 항목 자체를 생략)
...
- [핵심 포인트]의 설명 부분은 완결된 문장이 아니어도 됩니다. 명사형이나 짧은 절로 끝내도 됩니다.
