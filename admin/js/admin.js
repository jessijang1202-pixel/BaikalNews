...
// 승인받은 고정 템플릿에 별도로 들어감). 실제 등록한 고정 템플릿 기준으로
// 재보니 고정 문구가 약 103자뿐이라(전체 1,000자 한도 - 고정 103자 =
// 이론상 897자까지 가능), 여유를 넉넉히 두고 800자로 상향. 이 제한을
// 넘기면 알림톡 자체가 발송 불가능해지므로(단순히 "권장"이 아니라 기술적
// 요건), 초과 시 한 번 더 짧게 재생성을 시도하고, 그래도 넘으면 절대
// 그냥 보내지 말고 관리자가 직접 줄이도록 명확히 경고한다.
const KAKAO_BRIEFING_VAR_CHAR_LIMIT = 800;
const KAKAO_BRIEFING_VAR_CHAR_TARGET_MIN = 700; // 짧게 끝나는 것도 문제라 최소선도 둠
...
- 제목 중복을 없앤 만큼 아낀 글자수로, 최대한 많은 뉴스 항목(목표 9~11개 내외, 글자수 예산이 허락하는 한 최대한)을 다루십시오. 제목만으로 알 수 없는 내용은 추측하지 말고, 명백한 사실 위주로 작성하십시오.
