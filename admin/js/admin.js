...
async function renderArticlesList() {
  ...
  tbody.innerHTML = sortedArticles.map((art, i) => {
    ...
    if (art.status === 'published') {
      const snsDoneCls = art.snsWorkDone ? ' shorts-status-done' : '';
    ...
    if (shortsEligibleStatuses.includes(art.status)) {
      ...
      // video_ready도, shortsWorkDone도 전부 Supabase에 저장되는 값이라
      // 누가 어느 기기/브라우저에서 작업했든 모두에게 동일하게 보인다.
      const shortsDoneCls = (completedShorts || art.shortsWorkDone) ? ' shorts-status-done' : '';
...
// 기사관리 목록의 SNS/숏폼 버튼 완료 표시 -- 해당 기사로 SNS 이미지/카드뉴스
// "생성"을 하거나 그 중간 저장 버튼(SNS는 기사요약 저장, 숏폼은 대본 저장)을
// 누르면 버튼을 회색으로 반전시켜 목록에서 바로 알아볼 수 있게 한다. 여러
// 관리자가 함께 쓰는 화면이라 "내가" 했는지가 아니라 "누구든" 이 기사로
// 작업을 시작했는지를 모두가 보게 하는 게 목적이므로, 브라우저 로컬이 아니라
// Supabase(articles.sns_work_done / shorts_work_done)에 저장한다. 단순히 탭을
// 열어 기사를 선택만 해 본 경우는 반전하지 않는다.
async function markArticleWorkDone(articleId, kind) {
  if (!articleId || !window.SupabaseAdapter) return;
  await window.SupabaseAdapter.markArticleWorkDone(articleId, kind);
  renderArticlesList();
}
...
async function openLocalShortsDraft(localDraftId) {
  ...
  try {
    ...
    if (draft.hasOutroNarration) {
      const blob = await idbGetBlob(`${localDraftId}:narration:outro`);
      if (blob) {
        currentShortsProject.outroNarrationKey = `${localDraftId}:narration:outro`;
        currentShortsProject.outroNarrationUrl = URL.createObjectURL(blob);
        currentShortsProject.outroNarrationBase64 = await blobToBase64DataUrl(blob);
      } else {
        missingMedia.push("엔딩 구독 유도 나레이션");
      }
    }
    let cutIdx = 0;
...
async function generateShortsScript() {
  ...
  try {
    ...
    const backInstruction = neededAiCuts > 0
      ? `- 0:08~0:30 (이미지, 22초): ${neededAiCuts}개의 정지 이미지 컷을 작성하십시오. (전체 ${SHORTS_TARGET_CUT_COUNT}컷 중 ${backUploads.length}개는 관리자가 이미 준비한 자료를 사용하므로 나머지 ${neededAiCuts}개만 작성하면 됩니다.) 각 컷의 "prompt"는 한글 이미지 생성 프롬프트로, 맨 앞에 반드시 "[세로 9:16 비율, 1080x1920px]"라고 이미지 규격을 명시한 뒤 줄바꿈하고 이어서 장면(다큐멘터리 사진 스타일, 세로 구도)을 묘사하십시오 (veoPrompt와 마찬가지로, 관리자가 이 프롬프트를 그대로 복사해 외부 이미지 생성 도구에 붙여넣어 쓰는 경우가 있으므로 규격 정보가 프롬프트 문장 자체에 항상 포함되어야 합니다). "narration"에는 나레이션으로 읽을 자연스러운 한 문장을 쓰십시오 (소리 내어 읽었을 때 ${perCutDuration}초 안팎(약 ${targetNarrationChars}자 내외)에 끝나는 것을 목표로 하고, 내용이 중간에 끊기지 않도록 자연스럽게 마무리하십시오). 이 문장이 그대로 화면 자막으로도 표시되므로(코드가 자동으로 앞/뒤 절반으로 나눠 순서대로 띄웁니다), 이 컷의 화면(이미지)이나 기사에는 있지만 이 문장 안에서 직접 언급하지 않은 사실·수치·이름은 절대 넣지 마십시오 -- 나레이션에 없는 내용이 자막에만 나타나면 안 됩니다. 별도의 caption은 만들지 마십시오. 지속 시간(초, ${perCutDuration}초 내외)을 포함해야 합니다.`
...
반드시 다음 JSON 형식으로만 답하십시오. 백틱이나 다른 설명 없이 JSON 객체만 출력하십시오. "scriptMd"처럼 여러 줄로 작성하는 값 안의 줄바꿈은 반드시 \\n으로 이스케이프하여, 유효한 JSON 문자열 하나로 만드십시오 (실제 줄바꿈 문자를 문자열 안에 그대로 넣지 마십시오).
{
  ...
  "imageCuts": [
    { "prompt": "한글 이미지 프롬프트", "narration": "이 컷에서 나레이션으로 읽을 자연스러운 한 문장 (이 문장 그대로 화면 자막으로도 쓰이므로, 여기 없는 사실을 다른 곳에 넣지 마십시오)", "duration": ${perCutDuration} }
  ],
...
`;
    ...
    const aiCuts = (script.imageCuts || []).map(c => {
      // 자막을 나레이션과 별도로 AI가 새로 짓게 했더니, 나레이션에서
      // 언급하지 않은 기사 내용이 자막에만 나타나 나레이션과 자막이
      // 서로 안 맞는 문제가 있었다. 이제 자막은 별도로 짓지 않고,
      // 실제 나레이션 문장을 그대로 앞/뒤 절반으로 나눠 순서대로
      // 띄운다(findNearestSpaceSplit) -- 화면에 보이는 내용과 들리는
      // 내용이 항상 정확히 일치한다. c.caption1/c.caption은 이 방식이
      // 생기기 전 저장된 옛 응답을 위한 대비책일 뿐이다.
      const narrationText = (c.narration || c.caption1 || c.caption || '').trim();
      const [caption1, caption2] = findNearestSpaceSplit(narrationText, 0.5);
      return {
        prompt: c.prompt || '', caption: caption1 || narrationText, caption2: caption2 || '',
        narrationText,
...
async function generateShortsNarration() {
  ...
  try {
    ...
    // 구독 유도 엔딩 나레이션 -- 항상 고정 문구로 생성한다 (엔딩 이미지가
    // 아직 업로드 전이면 조립 단계에서 이 세그먼트 자체를 건너뛴다).
    if (statusEl) statusEl.textContent = "나레이션 생성 중... (엔딩 구독 유도)";
    await generateCutNarration(null, SHORTS_OUTRO_NARRATION_TEXT, voiceName, draftId, 'outro');
...
const SHORTS_KEN_BURNS_EFFECTS = ['zoom-in', 'zoom-out', 'pan-right', 'pan-diagonal-left'];
...
async function buildShortsAssets(project) {
  ...
  try {
    ...
    if (project.outroNarrationUrl) {
      try {
        const mix = ensureAudioMix();
        await mix.audioCtx.resume().catch(() => {});
        const resp = await fetch(project.outroNarrationUrl);
        outroNarrationBuffer = await mix.audioCtx.decodeAudioData(await resp.arrayBuffer());
      } catch (err) {
  ...
  return { front, images, audioCtx, mixDestination, hookNarrationBuffer, watermarkLogo, outro };
}
...
function drawShortsBrandWatermark(ctx, canvasW, canvasH, logoImg) {
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  const textWidth = ctx.measureText("바이칼TV").width;
  ...
  ctx.fillStyle = "#ffffff";
  ctx.fillText("바이칼TV", cursorX + textWidth / 2, centerY + 1);
  ctx.restore();
}
...
function drawShortsKenBurnsImage(ctx, img, progress, canvasW, canvasH, effect) {
  ...
  // decreasing=true -> coordinate shrinks over time (reveals more of the
  // right/bottom of the image); decreasing=false -> grows over time
  // (reveals more of the left/top). Range is centered on the normal
  // cover-fit position and capped to canvasSize*capFraction either way.
  const panAxis = (drawSize, canvasSize, capFraction, decreasing) => {
    const overflow = drawSize - canvasSize;
    const maxPan = Math.min(overflow, canvasSize * capFraction);
    const center = -overflow / 2;
    const half = maxPan / 2;
    return decreasing ? (center + half - maxPan * progress) : (center - half + maxPan * progress);
  };
...
async function runShortsTimelineInner(canvas, assets, project, { record } = {}) {
  ...
  await new Promise(async (resolve) => {
    ...
    if (assets.audioCtx && assets.mixDestination) {
      ...
      if (assets.outro && assets.outro.narrationBuffer) {
        const src = assets.audioCtx.createBufferSource();
        src.buffer = assets.outro.narrationBuffer;
        src.playbackRate.value = narrationSpeed;
        src.connect(assets.mixDestination);
        src.connect(assets.audioCtx.destination);
        src.start(base + totalDuration);
        try { src.stop(base + grandTotalDuration); } catch (err) { /* already stopped */ }
        scheduledSources.push(src);
      }
    }
...
[작성 지침]
- 리드 문단을 그대로 옮기지 마십시오.
- 절대 높임말(존댓말)을 쓰지 마십시오. "-습니다", "-합니다", "-했습니다", "-입니다", "-됩니다", "-라고 밝혔습니다" 같은 종결어미는 단 하나도 쓰면 안 됩니다.
- 반드시 "-다", "-했다", "-이다" 같은 평서형 종결이나 명사형 종결(예: "~ 논란", "~ 파장")로 끝맺으십시오. 뉴스 보도체가 아니라, 짧고 힘있는 문장으로 쓰십시오.
- 마크다운이나 추가 설명 없이 5줄 본문만 출력하십시오.`;

    const systemInstruction = "당신은 뉴스 기사를 SNS 포토뉴스 이미지용 5줄 요약으로 재구성하는 카피라이터입니다. 앞 3줄은 사실 전달, 뒤 2줄은 임팩트 있는 강조 문장으로 정확히 5줄만 작성하고, 번호·불릿·라벨 없이 줄바꿈으로만 구분하십시오. 절대 높임말(존댓말: -습니다/-합니다/-했습니다/-입니다)을 쓰지 말고, 반드시 평서형(-다/-했다) 또는 명사형 종결로 문장을 끝맺으십시오.";
