...
// 승인받은 고정 템플릿에 별도로 들어감). 알림톡 전체 글자수 한도(변수+고정
// 문구+줄바꿈 합쳐 1,000자)에서 고정 문구(약 125자)를 뺀 예산 안에서
// 안전 마진을 두고 650자로 잡음 -- 이 제한을 넘기면 알림톡 자체가 발송
// 불가능해지므로(단순히 "권장"이 아니라 기술적 요건), 초과 시 한 번 더
...
- (매우 중요) 각 뉴스 항목은 번호 뒤에 "제목 줄"과 "설명 줄"을 따로 나누지 말고, 반드시 줄바꿈 없이 하나의 문장(또는 이어지는 두 문장)으로 바로 핵심 사실을 전달하십시오. 예를 들어 "1. 밭일하던 100세 할머니 숨진 채 발견\n밭일을 하던 100세 할머니가 숨진 채 발견되었습니다." 처럼 제목을 쓰고 그 아래 줄에서 같은 내용을 다시 풀어 쓰는 방식은 같은 내용이 중복되어 글자를 낭비하므로 절대 금지합니다. 대신 "1. 밭일하던 100세 할머니가 숨진 채 발견됐으며, 발견 당시 체온은 42.2도였습니다."처럼 번호 뒤에 바로 한 줄로 이어서 쓰십시오.
- 제목 중복을 없앤 만큼 아낀 글자수로, 최대한 많은 뉴스 항목(목표 9~11개 내외, 글자수 예산이 허락하는 한 최대한)을 다루십시오. 항목 사이에 빈 줄을 넣지 말고 줄바꿈만 사용하십시오. 제목만으로 알 수 없는 내용은 추측하지 말고, 명백한 사실 위주로 작성하십시오.
